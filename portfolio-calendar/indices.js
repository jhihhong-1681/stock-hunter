...
window.INDEX_HISTORY = [
  ...
  {
    date: "2026-08-27",
    taiex: 0.31,    // 台股加權指數（Yahoo Finance歷史收盤45,975.22，較前一日45,832.62上漲142.60點/+0.31%；WebSearch誤抓到不相干的過期資料「43,634.19/-0.05%」，正負號與數字皆不符，比對失敗，採用Yahoo Finance）
    sp500: 0.72,    // S&P 500（Yahoo Finance收盤7,730.99/+0.72% 與 WebSearch另一數字+0.57%~+0.7% 差距在容許範圍內，交叉比對成功）
    nasdaq: 1.57,   // 那斯達克綜合指數（Yahoo Finance收盤26,541.35/+1.57% 與 WebSearch +1.4% 差0.17pp在容許範圍內，交叉比對成功）
    sox: 2.33       // 費城半導體指數（Yahoo Finance收盤11,882.17/+2.33%；次要來源WebSearch查無可比對的當日收盤數字，採用Yahoo Finance）
  },
  {
    date: "2026-09-03",
    taiex: -0.67,   // 台股加權指數（8/28~9/2排程中斷未記錄，此為排程重啟後第一筆；Yahoo Finance history頁未直接列出9/3該列，但用前一筆9/2收盤46,164.72反推與WebSearch「收盤45,857.66/-0.67%」完全吻合，交叉比對成功）
    sp500: 1.05,    // S&P 500（finance.yahoo.com/quote/^GSPC被中斷兩次無法取得，改用stockanalysis.com的SPY ETF收盤+1.05%當替代來源，與WebSearch其中一筆「+0.99%」差0.06pp在容許範圍內，交叉比對成功）
    nasdaq: 1.40,   // 那斯達克綜合指數（Yahoo Finance收盤26,584.06/+1.40% 與 WebSearch 26,599.06/+1.45% 差距在容許範圍內，交叉比對成功）
    sox: 0.11       // 費城半導體指數（Yahoo Finance收盤11,352.13/+0.11% 與 WebSearch 11,338.21/-0.01% 正負號不一致，比對失敗，採用Yahoo Finance）
  },
  {
    date: "2026-09-04",
    taiex: 1.51,    // 台股加權指數（Yahoo Finance歷史收盤46,551.13，較前一日45,857.66上漲693.47點/+1.51% 與 WebSearch新聞「收盤46,551.13/上漲693.47點/+1.51%」完全吻合）
    sp500: -0.38,   // S&P 500（Yahoo Finance收盤7,718.60/-0.38% 與 WebSearch同一數字完全吻合）
    nasdaq: -0.29,  // 那斯達克綜合指數（Yahoo Finance收盤26,506.99/-0.29% 與 WebSearch同一數字完全吻合）
    sox: 3.38       // 費城半導體指數（Yahoo Finance收盤11,735.26/+3.38%；次要來源WebSearch抓到不相干/錯誤的數字(8,137.86)，比對失敗，採用Yahoo Finance）
  },
  {
    date: "2026-09-07",
    taiex: 1.67,    // 台股加權指數（9/7非美股假期照常交易；Yahoo Finance收盤47,326.27，較前一日46,551.13上漲775.14點/+1.67% 與 WebSearch新聞「上漲775.14點/收47,326.27/+1.67%」完全吻合）
    sp500: 0.00,    // S&P 500（9/7美股勞動節休市，沿用上一筆，無新收盤）
    nasdaq: 0.00,   // 那斯達克綜合指數（9/7美股勞動節休市，沿用上一筆，無新收盤）
    sox: 0.00       // 費城半導體指數（9/7美股勞動節休市，沿用上一筆，無新收盤）
  },
  {
    date: "2026-09-08",
    taiex: -0.47,   // 台股加權指數（Yahoo Finance歷史收盤47,105.78，較前一日47,326.27下跌220.49點/-0.47% 與 WebSearch新聞「尾盤拉回220點收47,105點」完全吻合）
    sp500: -0.58,   // S&P 500（Yahoo Finance歷史收盤7,673.52，較前一日(9/4)7,718.60下跌45.08點/-0.58% 與 WebSearch「下跌44.94點/-0.58%」完全吻合）
    nasdaq: -0.32,  // 那斯達克綜合指數（Yahoo Finance收盤26,421.41/-0.32% 與 WebSearch同一數字完全吻合）
    sox: 1.30       // 費城半導體指數（Yahoo Finance收盤11,887.87/+1.30%；次要來源WebSearch只查到9/4舊資料，查無可比對的當日收盤數字，採用Yahoo Finance）
  },
  {
    date: "2026-09-09",
    taiex: 0.16,    // 台股加權指數（Yahoo Finance歷史收盤47,183.36，較前一日47,105.78上漲77.58點/+0.16% 與 WebSearch「收盤47,194.95/+0.19%」差0.03pp在容許範圍內，交叉比對成功）
  ...
  {
    date: "2026-09-14",
    taiex: -0.70,   // 台股加權指數（Yahoo Finance昨收45,862.52，較前一交易日9/11收盤46,184.85下跌322.33點/-0.70% 與 WebSearch同一數字「下跌322.33點/-0.70%收45,862.52點」完全吻合）
    sp500: -0.48,   // S&P 500（Yahoo Finance收盤7,619.98/-0.48% 與 WebSearch同一數字完全吻合）
    nasdaq: -0.56,  // 那斯達克綜合指數（Yahoo Finance收盤26,186.41/-0.56% 與 WebSearch同一數字完全吻合）
    sox: -5.86      // 費城半導體指數（Yahoo Finance收盤11,131.28/-5.86%；次要來源WebSearch查無可比對的當日收盤數字，採用Yahoo Finance）
  },
  {
    date: "2026-09-15",
    taiex: -0.77,   // 台股加權指數（Yahoo Finance收盤45,511.49，較前一日45,862.52下跌351.03點/-0.77% 與 WebSearch新聞「收在45,511.49點/下跌351.03點/-0.77%」完全吻合）
    sp500: -0.45,   // S&P 500（Yahoo Finance收盤7,585.73/-0.45%；次要來源WebSearch抓到9/14舊資料7,619.98/-0.48%，比對失敗，採用Yahoo Finance）
    nasdaq: -0.78,  // 那斯達克綜合指數（Yahoo Finance收盤25,981.57/-0.78%；次要來源WebSearch回傳明顯錯誤/不相干的數字，採用Yahoo Finance）
    sox: 0.40       // 費城半導體指數（Yahoo Finance收盤11,175.55/+0.40%；次要來源WebSearch查無可比對的當日收盤數字，採用Yahoo Finance）
  },
  {
    date: "2026-09-16",
    taiex: 0.74,    // 台股加權指數（Yahoo Finance收盤45,848.90，較前一日45,511.49上漲337.41點/+0.74% 與 WebSearch新聞「收盤漲337.41點/45,848.90點/漲幅0.74%」完全吻合）
    sp500: -0.45,   // S&P 500（Yahoo Finance收盤7,551.81/-0.45% 與 WebSearch同一數字完全吻合）
    nasdaq: -0.01,  // 那斯達克綜合指數（Yahoo Finance收盤25,978.42/-0.01% 與 WebSearch同一數字完全吻合）
    sox: 0.63       // 費城半導體指數（Yahoo Finance收盤11,246.11/+0.63%；次要來源WebSearch查無可比對的當日收盤數字，採用Yahoo Finance）
  },
  {
    date: "2026-09-17",
    taiex: 0.96,    // 台股加權指數（Yahoo Finance收盤46,288.00，較前一日45,848.90上漲439.10點/+0.96% 與 WebSearch新聞「收在46288點/漲439.10點/漲幅0.96%」完全吻合）
    sp500: 1.14,    // S&P 500（Yahoo Finance收盤7,637.76/+1.14% 與 WebSearch +1.12%/收盤7,637.72 差0.02pp在容許範圍內，交叉比對成功）
    nasdaq: 1.69,   // 那斯達克綜合指數（Yahoo Finance收盤26,418.30/+1.69% 與 WebSearch +1.6% 差0.09pp在容許範圍內，交叉比對成功）
    sox: 3.14       // 費城半導體指數（Yahoo Finance收盤11,599.49/+3.14%；次要來源WebSearch查無可比對的當日收盤數字，採用Yahoo Finance）
  },
  {
    date: "2026-09-18",
    taiex: 1.93,    // 台股加權指數（Yahoo Finance收盤47,180.75，較前一日46,288.00上漲892.75點/+1.93% 與 WebSearch新聞「收盤漲892.75點/47180.75點/漲幅1.93%」完全吻合）
    sp500: 0.17,    // S&P 500（Yahoo Finance收盤7,650.50/+0.17% 與 WebSearch +0.16~0.17% 完全吻合）
    nasdaq: 0.39,   // 那斯達克綜合指數（Yahoo Finance收盤26,522.55/+0.39%；次要來源WebSearch回報自相矛盾的「+104.24點但0.00%」，比對失敗，採用Yahoo Finance）
    sox: 2.78       // 費城半導體指數（Yahoo Finance收盤11,921.69/+2.78%；次要來源WebSearch回報「持平0.00%」與Yahoo Finance差距超過0.3pp，比對失敗，採用Yahoo Finance）
  },
  {
    date: "2026-09-21",
    taiex: 1.14,    // 台股加權指數（Yahoo Finance收盤47,718.84，較前一日47,180.75上漲538.09點/+1.14% 與 WebSearch新聞「漲538.09點/漲幅1.14%」完全吻合）
    sp500: 1.49,    // S&P 500（Yahoo Finance收盤7,764.70/+1.49% 與 WebSearch收盤7,764.80/+1.55% 差0.06pp在容許範圍內，交叉比對成功）
    nasdaq: 2.26,   // 那斯達克綜合指數（Yahoo Finance收盤27,122.09/+2.26% 與 WebSearch同一數字完全吻合）
    sox: 4.29       // 費城半導體指數（Yahoo Finance收盤12,433.17/+4.29%；次要來源WebSearch查無可比對的當日收盤數字，採用Yahoo Finance）
  },
  {
    date: "2026-09-22",
    taiex: 0.17,    // 台股加權指數（Yahoo Finance歷史表缺9/22這一列；以Yahoo 9/21收盤47,718.84為基準，WebSearch/Newtalk新聞「收在47,800.17點/漲81.33點/漲幅0.17%」，81.33÷47,718.84=+0.17%吻合；搜尋摘要誤寫0.60%已排除）
    sp500: -0.00,   // S&P 500（Yahoo Finance收盤7,764.64/前收7,764.70，-0.06點/-0.00% 與 WebSearch同一數字完全吻合）
    nasdaq: 0.45,   // 那斯達克綜合指數（Yahoo Finance收盤27,244.28/+0.45% 與 WebSearch同一數字完全吻合）
    sox: 2.06       // 費城半導體指數（Yahoo Finance收盤12,689.82/+2.06%；次要來源WebSearch只查到同價位12,689.8、無百分比，採用Yahoo Finance）
  },
  {
    date: "2026-09-23",
    taiex: 0.75,    // 台股加權指數（Yahoo Finance收盤48,157.29；Yahoo歷史表仍缺9/22列，以9/22收盤47,800.17為基準+357.12點=+0.75%，與WebSearch新聞「漲357.12點/48,157.29/漲幅0.75%」吻合）
